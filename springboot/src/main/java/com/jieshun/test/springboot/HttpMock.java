...
public class HttpMock {
	protected static Logger logger  = LoggerFactory.getLogger(HttpMock.class);
	...
	//接口mock程序
	@RequestMapping("/**")
	String httpMock(HttpServletRequest req, HttpServletResponse res) throws UnsupportedEncodingException {
		//统计请求次数，防止线程安全，加同步锁
		synchronized(this)
			{
				requestCount++;
		...
		try {
			...
			logger.info("接收到的请求的IP："+req.getRemoteHost()+" 请求内容为：-----"+JSON.toJSONString(req.getParameterMap())+sb.toString()+"---请求次数："+requestCount);
